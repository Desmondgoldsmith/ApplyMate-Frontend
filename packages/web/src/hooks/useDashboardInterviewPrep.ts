...
import { dashboardInterviewPrepQueryKey } from '@/lib/today-plan';

export function useDashboardInterviewPrep(params: {
  ...
  const ffm =
    ...
    params.focusFeedMaxItems <= 100
      ...
      : undefined;
  return useQuery({
    queryKey: dashboardInterviewPrepQueryKey({
      cvProfileId: cv,
      timezone: tz,
      focusFeedMaxItems: ffm,
    }),
    queryFn: () =>
      api.dashboard.getInterviewPrep({
        cvProfileId: cv,
        timezone: tz,
