...
import { dedupePreviewSectionKeys, professionalSectionRank } from '@/lib/cvSectionProfessionalOrder';
...
function mergeNewSectionsIntoData(prev: CVBuilderData, incoming: CVBuilderData): CVBuilderData {
  ...
  const seedIfEmpty = <T,>(key: keyof CVBuilderData, prevArr: T[], incArr: T[]) => {
  ...
  if (!prev.parsedCustomSections.length && incoming.parsedCustomSections.length) {
...
const TEMPLATE_FIELD_CONFIG = {
  ...
  showPhotoUpload: ['europass-classic', 'europass-modern', 'french', 'german'] as CvTemplateId[],
  showHobbies: ['french', 'german'] as CvTemplateId[],
  showReferences: ['classic', 'modern', 'creative', 'professional', 'uk', 'german'] as CvTemplateId[],
  showCefrLanguageBreakdown: ['europass-classic', 'europass-modern'] as CvTemplateId[],
} as const;
...
function spellAggregatesFromByField(byField: Record<string, CvSpellIssue[]>): {
  ...
  for (const [composite, list] of Object.entries(byField)) {
    ...
    for (const issue of list) {
      ...
      spellIssueEntriesBySection[sectionKey] = [...(spellIssueEntriesBySection[sectionKey] ?? []), enriched];
    }
  }
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? 'border-[#00C9B1]' : 'border-white/[0.08] hover:border-white/20',
      )}
    >
      <div
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/85 to-transparent px-2 py-1.5">
        <p className="truncate text-[10px] font-semibold text-white">{TEMPLATE_LABELS[tid]}</p>
...
function readHeaderPreviewStorage(profileId: string | null | undefined): HeaderPreviewSettings {
  if (!profileId?.trim() || typeof window === 'undefined') return DEFAULT_HEADER_PREVIEW;
  try {
    const raw = window.sessionStorage.getItem(`applymate:cv:headerPreview:${profileId.trim()}`);
    if (!raw) return DEFAULT_HEADER_PREVIEW;
    const o = JSON.parse(raw) as Partial<HeaderPreviewSettings> & { showLink?: boolean; showExtraLink?: boolean };
    ...
    return {
      ...
      showLinkedIn: o.showLinkedIn ?? legacyLink ?? DEFAULT_HEADER_PREVIEW.showLinkedIn,
      showGithub: o.showGithub ?? legacyLink ?? DEFAULT_HEADER_PREVIEW.showGithub,
      showWebsiteToggle: o.showWebsiteToggle ?? DEFAULT_HEADER_PREVIEW.showWebsiteToggle,
      showPortfolioToggle: o.showPortfolioToggle ?? o.showExtraLink ?? DEFAULT_HEADER_PREVIEW.showPortfolioToggle,
    };
  } catch {
...
function writeHeaderPreviewStorage(profileId: string | null | undefined, next: HeaderPreviewSettings) {
  if (!profileId?.trim() || typeof window === 'undefined') return;
  try {
    window.sessionStorage.setItem(`applymate:cv:headerPreview:${profileId.trim()}`, JSON.stringify(next));
  } catch {
    /* ignore */
  }
}
 ...
 */
function orderSectionRowIdsByPreviewKeys(allSections: CVSectionRecord[], previewKeys: string[]): string[] {
...
function accordionKeyProfessionalRank(accordionKey: string, sections: CVSectionRecord[]): number {
  const row = resolveSectionForAccordion(accordionKey, sections);
  if (row?.type) return professionalSectionRank(row.type);
  if (accordionKey === 'custom-legacy') return professionalSectionRank('custom');
 ...
 */
function mergePreviewSectionOrder(prev: string[], sections: CVSectionRecord[]): string[] {
  ...
  const sortedNew = [...newKeys].sort(
    (a, b) => accordionKeyProfessionalRank(a, sections) - accordionKeyProfessionalRank(b, sections),
  );
...
export function CVBuilder({
  ...
  const [data, setData] = useState<CVBuilderData>(() =>
    initialData ?? emptyCVBuilderData({ email: emailDefault, name: user?.name }),
  );
  const [expanded, setExpanded] = useState<Set<string>>(() => {
    const seed = initialData ?? emptyCVBuilderData({ email: emailDefault, name: user?.name });
    const s = new Set(['personal', 'experience', 'education', 'skills']);
    if (TEMPLATE_FIELD_CONFIG.showReferences.includes(selectedTemplate) && seed.references.length > 0) {
      s.add('references');
    }
    return s;
  });
  ...
  const [allowIncompletePreviewBadges, setAllowIncompletePreviewBadges] = useState(
    () => !deferIncompletePreviewBadges,
  );
  useEffect(() => {
    if (!deferIncompletePreviewBadges) return undefined;
    const id = window.setTimeout(() => setAllowIncompletePreviewBadges(true), 60_000);
    return () => window.clearTimeout(id);
  }, [deferIncompletePreviewBadges]);

  useEffect(() => {
    if (deferIncompletePreviewBadges && dirty) setAllowIncompletePreviewBadges(true);
  }, [deferIncompletePreviewBadges, dirty]);
  const [saveStatus, setSaveStatus] = useState<CvBuilderSaveStatus>('idle');
  const [focusedPreviewSection, setFocusedPreviewSection] = useState<string | null>(null);
  const [focusedEntryId, setFocusedEntryId] = useState<string | null>(null);
  const [focusedEntrySection, setFocusedEntrySection] = useState<string | null>(null);
  const [knownSectionTypes, setKnownSectionTypes] = useState<Set<string>>(
    () => new Set(existingSections?.map((s) => s.type) ?? []),
  );
  const [headerPreview, setHeaderPreviewState] = useState<HeaderPreviewSettings>(DEFAULT_HEADER_PREVIEW);
  const [aiPending, setAiPending] = useState<string | null>(null);
  const [removeTarget, setRemoveTarget] = useState<{ kind: string; id: string } | null>(null);
  const [dragId, setDragId] = useState<string | null>(null);
  const [editorWidthPct, setEditorWidthPct] = useState(() => readStoredCvSplitPct());
  ...
  const [previewSectionOrder, setPreviewSectionOrder] = useState<string[]>(() => previewOrderFromSections(existingSections));
  const [reorderPending, setReorderPending] = useState(false);
  const [spellIssuesBySection, setSpellIssuesBySection] = useState<Record<string, number>>({});
  const [spellIssueEntriesBySection, setSpellIssueEntriesBySection] = useState<Record<string, CvSpellIssue[]>>({});
  const [spellIssuesByField, setSpellIssuesByField] = useState<Record<string, CvSpellIssue[]>>({});
  const [isSpellChecking, setIsSpellChecking] = useState(false);
  const [visibilityPendingAccordion, setVisibilityPendingAccordion] = useState<string | null>(null);
  const lastConfirmedPreviewOrderRef = useRef<string[]>(previewOrderFromSections(existingSections));
  ...
  const showTripleShell = mode === 'dashboard' && tripleColumn != null && desktopLg;
  ...
  useEffect(() => {
    ...
    setPreviewSectionOrder((prev) => mergePreviewSectionOrder(prev, existingSections));
  ...
  useEffect(() => {
    ...
    if (previous === null) {
      ...
      for (const t of optionalSectionPresence) optionalSeedDoneRef.current.add(t);
      return;
    }
    /** Find genuinely new types (added by user via "Sections" modal, not present before). */
    const newlyAdded: string[] = [];
    for (const t of optionalSectionPresence) {
      if (!previous.has(t) && !optionalSeedDoneRef.current.has(t)) newlyAdded.push(t);
    }
    previousOptionalPresenceRef.current = new Set(optionalSectionPresence);
    ...
    setData((prev) => {
      ...
      seedNew('certifications', prev.certifications.length > 0, () => {
        const n = touch();
        n.certifications = [{ id: newLocalId(), name: '', issuer: '', date: '', url: '' }];
      });
      seedNew('projects', prev.projects.length > 0, () => {
        const n = touch();
        n.projects = [{ id: newLocalId(), name: '', description: '', technologies: [], url: '', bullets: '' }];
      });
      seedNew('languages', prev.languages.length > 0, () => {
        const n = touch();
        n.languages = [{ id: newLocalId(), language: '', proficiency: 'Fluent' }];
      });
      seedNew('achievements', prev.achievements.length > 0, () => {
        const n = touch();
        n.achievements = [{ id: newLocalId(), title: '', issuer: '', date: '', detail: '' }];
      });
      seedNew('references', prev.references.length > 0, () => {
        const n = touch();
        n.references = [{ id: newLocalId(), name: '', title: '', company: '', email: '', phone: '' }];
      });
  ...
  const runCvAssistantWithTarget = useCallback(
    async (
      ...
      const targetSection = assistantTargetSectionFromAccordionKey(key, sectionsRef.current);
  ...
  const sectionsMergeKey = useMemo(
    () => existingSections.map((s) => `${s.id}:${s.type}:${s.order}:${s.hidden === true ? 1 : 0}`).join('|'),
    [existingSections],
  );
  ...
  }, [existingSections, mode, knownSectionTypes, data.personal.email, data.personal.name]);

  const sectionsVisibilityFingerprint = useMemo(
    () => existingSections.map((s) => `${s.id}:${s.type}:${s.hidden === true ? 1 : 0}`).join('|'),
    [existingSections],
  );
  ...
  useEffect(() => {
     ...
     */
    for (const opt of ['projects', 'certifications', 'achievements', 'languages', 'references']) {
      next[opt] = false;
    }
    for (const s of list) {
      const key = accordionKeyFromSection(s);
      ...
      if (key === 'summary' || key === 'experience' || key === 'education' || key === 'skills') {
        next[key] = true;
        continue;
      }
  ...
  const showIntlFields = TEMPLATE_FIELD_CONFIG.showInternationalPersonalFields.includes(template as CvTemplateId);
  const showPhotoUpload = TEMPLATE_FIELD_CONFIG.showPhotoUpload.includes(template as CvTemplateId);
  const showHobbies = TEMPLATE_FIELD_CONFIG.showHobbies.includes(template as CvTemplateId);
  const showReferences = TEMPLATE_FIELD_CONFIG.showReferences.includes(template as CvTemplateId);
  const showCefr = TEMPLATE_FIELD_CONFIG.showCefrLanguageBreakdown.includes(template as CvTemplateId);

  const onSplitResizePointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    e.preventDefault();
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      if (w < 64) return;
      const next = clampCvSplitPct(startPct + ((ev.clientX - startX) / w) * 100);
    ...
    const onUp = (ev: PointerEvent) => {
      ...
      try {
        window.localStorage.setItem(CV_SPLIT_STORAGE_KEY, String(clampCvSplitPct(editorWidthPctRef.current)));
      } catch {
        /* ignore */
      }
    };
  ...
  const onSplitHandleDoubleClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    e.preventDefault();
  ...
  const update = useCallback((patch: Partial<CVBuilderData> | ((d: CVBuilderData) => CVBuilderData)) => {
    setDirty(true);
  ...
  useEffect(() => {
    if (mode !== 'dashboard') return;
    if (!serverHydrateNonce || serverHydrateNonce === lastServerHydrateNonceRef.current) return;
  ...
  }, [mode, serverHydrateNonce, emailDefault, user?.name, selectedTemplate, isTailorView, dirty]);
  ...
  const experienceItems = Array.isArray(rawExperienceItems) ? rawExperienceItems : [];
  ...
  const skillCategories = Array.isArray(rawSkillCategories) ? rawSkillCategories : [];

  const sectionDone = useMemo(() => {
    const hasExp = experienceItems.some((x) => x.title.trim() && x.company.trim());
    const hasSkills = skillCategories.some((c) =>
      (Array.isArray(c.skills) ? c.skills : []).some((s) => s.trim()),
    );
    const out: Record<string, boolean> = {
      personal: Boolean(data.personal.name.trim() && data.personal.email.trim()),
      experience: hasExp,
      ...
      projects: !optionalSectionPresence.has('projects') || data.projects.some((p) => p.name.trim()),
      achievements:
      ...
      certifications:
        ...
        data.certifications.some((c) => c.name.trim() && (c.issuer.trim() || c.date.trim())),
      languages: !optionalSectionPresence.has('languages') || data.languages.some((l) => l.language.trim()),
      references:
        !optionalSectionPresence.has('references') ||
        !showReferences ||
        data.references.some(
          (r) => r.name.trim() || r.title.trim() || r.company.trim() || r.email.trim() || r.phone.trim(),
        ),
      // Legacy placeholder row should never drive "incomplete" chips.
      'custom-legacy': true,
    };
    for (const b of data.parsedCustomSections) {
      out[`parsed-${b.sectionId}`] = b.items.some((i) => i.text.trim() || i.subItems.length > 0);
    }
    return out;
  }, [data, educationItems, experienceItems, optionalSectionPresence, showReferences, skillCategories]);
  ...
  useEffect(() => {
    ...
    const onHide = (e: Event) => {
      ...
      const isCoreId = lowered === 'summary' || lowered === 'experience' || lowered === 'education' || lowered === 'skills';
      if (isCoreId) return;
      ...
      if (rowId && profileId?.trim()) {
        ...
        if (t === 'summary' || t === 'experience' || t === 'education' || t === 'skills') return;
        void api.cv.removeSection(rowId, profileId)
          .then(async () => {
          ...
          .catch((e) => {
            try {
              toastRef.current?.error(getApiErrorMessage(e) || 'Failed to delete section');
            } catch {
              /* noop */
            }
          });
  ...
  const sectionLabelMap = useMemo<Record<string, string>>(() => {
    ...
    for (const b of data.parsedCustomSections) {
      out[`parsed-${b.sectionId}`] = (b.title || 'Custom section').trim() || 'Custom section';
    }
    return out;
  ...
  const missingFields = useMemo<CVBuilderMissingField[]>(() => {
    ...
    const push = (sectionKey: string, fieldPath: string, fieldLabel: string) => {
      if (hiddenSectionIds.has(sectionKey)) return;
      out.push({ sectionKey, sectionLabel: sectionLabelMap[sectionKey] ?? sectionKey, fieldPath, fieldLabel });
    };
    ...
    if (optionalSectionPresence.has('projects') && !data.projects.some((p) => p.name.trim())) {
      push('projects', 'items', 'Add at least one project name');
    }
    if (optionalSectionPresence.has('achievements') && !data.achievements.some((a) => a.title.trim() || a.issuer.trim())) {
      push('achievements', 'items', 'Add at least one achievement');
    }
    if (optionalSectionPresence.has('certifications') && !data.certifications.some((c) => c.name.trim() && (c.issuer.trim() || c.date.trim()))) {
      push('certifications', 'items', 'Add at least one certification with issuer or date');
    }
    if (optionalSectionPresence.has('languages') && !data.languages.some((l) => l.language.trim())) {
      push('languages', 'items', 'Add at least one language');
    }
    if (
      optionalSectionPresence.has('references') &&
      showReferences &&
      !data.references.some((r) => r.name.trim() || r.title.trim() || r.company.trim() || r.email.trim() || r.phone.trim())
    ) {
      push('references', 'items', 'Add at least one reference contact');
    }
    return out;
  }, [data, educationItems, experienceItems, hiddenSectionIds, optionalSectionPresence, sectionLabelMap, showReferences, skillCategories]);
  ...
  const previewIncompleteSectionIds = useMemo(() => {
    if (!deferIncompletePreviewBadges || allowIncompletePreviewBadges) return incompleteSectionIds;
    return EMPTY_INCOMPLETE_SECTION_IDS;
  }, [allowIncompletePreviewBadges, deferIncompletePreviewBadges, incompleteSectionIds]);
    ...
    (issue: CvSpellIssue) => {
      void (async () => {
        ...
        if (typeof start !== 'number' || typeof end !== 'number') {
          toast.error('This suggestion cannot be applied automatically. Edit the text manually.');
          return;
        }
        const suggestion = (issue.suggestion ?? issue.suggestions?.[0] ?? '').trim();
        if (!suggestion) {
        ...
        const mergeSpellField = (mapped: CvSpellIssue[]) => {
          ...
          const nextByField = { ...spellIssuesByFieldRef.current, [key]: mapped };
        ...
        const runApply = (forceRefresh: boolean) =>
          api.cv.applySpellcheck(pid, {
            ...
            fieldPath,
            text: getCvBuilderSectionFieldText(dataRef.current, sectionKey, fieldPath),
            issueId,
            sourceTextHash,
            start,
            end,
        ...
        try {
          ...
          if (typeof appliedText === 'string') {
            setData((prev) => setCvBuilderSectionFieldText(prev, sectionKey, fieldPath, appliedText));
            setDirty(true);
          } else if (result.alreadyApplied !== true) {
            setData((prev) => {
              const cur = getCvBuilderSectionFieldText(prev, sectionKey, fieldPath);
              const merged = cur.slice(0, start) + suggestion + cur.slice(end);
              return setCvBuilderSectionFieldText(prev, sectionKey, fieldPath, merged);
            });
            setDirty(true);
          }
          ...
          if (axios.isAxiosError(e) && e.response?.status === 409) {
            try {
              ...
              mergeSpellField(mapped);
              toast.info('Underlines were refreshed; the text changed since the last spell check.');
              return;
            } catch {
              toast.error('Your edits conflict with the spell checker. Run Check spelling again.');
              return;
            }
          }
  ...
  useEffect(() => {
    ...
    const onApply = (e: Event) => {
      const issue = (e as CustomEvent).detail?.issue as CvSpellIssue | undefined;
    ...
    window.addEventListener('cv:spell-issue:dismiss', onDismiss as EventListener);
    return () => {
      window.removeEventListener('cv:spell-issue:apply', onApply as EventListener);
      window.removeEventListener('cv:spell-issue:dismiss', onDismiss as EventListener);
    };
  ...
  const cvEditContextValue = useMemo(
    () => ({
      ...
      cvAssistantClarificationQuestion: cvAssistantClarificationQuestion ?? null,
    }),
    [
      mode,
      data,
  ...
  const toggleSectionVisibility = useCallback(
    async (accordionId: string) => {
      ...
      try {
        ...
        void queryClient.invalidateQueries({ queryKey: ['cv', 'score', profileId], exact: true });
        void queryClient.invalidateQueries({ queryKey: cvSuggestionsQueryKey(profileId), exact: true });
      } catch {
  ...
  const sectionVisibilityUi = useCallback(
    (accordionId: string) => {
      const isCore =
        ...
        accordionId === 'education' ||
        accordionId === 'skills';
      const isVisible = isCore ? true : sectionVisibility[accordionId] !== false;
      return {
  ...
  const handlePreviewReorderSections = useCallback(
    async (nextOrder: string[]) => {
      ...
      if (allSections.length === 0) {
        ...
        toast.error('Your CV sections are still loading. Wait a moment, then try reordering again.');
        return;
      }

      const orderedIds = orderSectionRowIdsByPreviewKeys(allSections, dedupedOrder);
      if (orderedIds.length !== allSections.length) {
        setPreviewSectionOrder(lastConfirmedPreviewOrderRef.current);
        toast.error('Could not sync section order with the server. Please refresh the page.');
        return;
      }
  ...
  const tplPills = <div className="flex flex-wrap gap-2">{renderTemplatePills()}</div>;

  const runAiSummary = async () => {
    const title = normalizeText(data.personal.headline as unknown).trim();
    const hasExp = experienceItems.some((x) => x.title.trim() && x.company.trim());
    const skillStr = skillCategories.flatMap((c) => c.skills).filter(Boolean).join(', ');
    ...
    const expLines = experienceItems
      ...
      .slice(0, 4)
      .map((x) => {
        const span = x.current ? `${x.startDate}–Present` : `${x.startDate}–${x.endDate}`;
    ...
    const summaryForApi = [
      ...
      (normalizeText(data.summary?.text as unknown) ?? '').trim() ? `Current draft to improve:\n${normalizeText(data.summary?.text as unknown).trim()}` : '',
    ]
      .filter(Boolean)
      .join('\n\n')
      .trim()
    ...
    try {
      ...
      try {
        if (summaryForApi.length < 20) {
          toast.error('Add a bit more detail (summary prompt too short for AI).');
          return;
        }
        next = await api.cv.improveSummary({ summary: summaryForApi, context: contextHint });
      } catch {
        const bullet =
        ...
        const variants = await api.cv.rewriteBullet({
          bullet,
          context: 'Rewrite as a 2–3 sentence first-person CV summary from the facts above.'.slice(0, 100),
        });
        next = variants[0];
      }
      if (next) {
        ...
        if (profileId?.trim()) {
          try {
            await commitAcceptedStructuredDraft({
              ...
              mutation: () => api.cv.acceptGeneratorSummary(profileId.trim(), { text }),
              onRehydrated: () => onAiStructuredPersisted?.(),
            });
            toast.success('Summary saved to your CV');
          } catch (e) {
            toast.error(getApiErrorMessage(e) || 'Could not save summary. Try again.');
          }
        } else {
          update({ summary: { text } });
  ...
  const runAiBullets = async (jobId: string, bulletText: string) => {
    ...
    if (trimmed.length < 10) {
      toast.error('Write at least 10 characters in this bullet before asking AI to improve it.');
      return null;
    }
    setAiPending(jobId);
    try {
      ...
      if (profileId?.trim()) {
        ...
        try {
          ...
          toast.error(getApiErrorMessage(e) || 'Could not save bullet. Try again.');
          return null;
        }
      }
  ...
  const jumpToSection = useCallback((sid: string, itemId?: string, opts?: { scrollForm?: boolean }) => {
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => scrollAttempt(0)));
  }, [mode]);
  ...
  useEffect(() => {
    if (!isTailorView || !tailorHighlightSectionId?.trim() || !tailorHighlightNonce) return;
    ...
    const flashClass =
      tailorHighlightAction === 'reverted' ? 'tailor-revert-flash' : 'tailor-accept-flash';
    el.classList.add(flashClass);
    const t = window.setTimeout(() => el.classList.remove(flashClass), 2600);
    return () => window.clearTimeout(t);
  }, [isTailorView, tailorHighlightSectionId, tailorHighlightNonce, tailorHighlightAction, jumpToSection]);

  useEffect(() => {
    let spellIssueCount = 0;
    let grammarIssueCount = 0;
    for (const [sectionKey, issues] of Object.entries(spellIssueEntriesBySection)) {
      if (hiddenSectionIds.has(sectionKey)) continue;
      for (const issue of issues) {
        if (issue.type === 'grammar' || issue.type === 'style') grammarIssueCount += 1;
  ...
  const runBulkSpellCheck = useCallback(async (opts?: { silent?: boolean; attempt?: number }) => {
    ...
    try {
      const res: CvSpellcheckBulkResult = await api.cv.checkSpellingBulk(profileId, { language: 'en' });
      ...
      if (!silent) {
        ...
        else toast.success(`Found ${total} potential issue${total === 1 ? '' : 's'}`);
      }
    } catch (e) {
      ...
      if (silent) {
        ...
        if (typeof console !== 'undefined') console.warn('cv:spellcheck:silent-error', msg);
  ...
  useEffect(() => {
    ...
    const issuesSnapshot = Object.values(spellIssueEntriesBySectionRef.current).flat();
    ...
    if (!applicable.length) {
      toast.error('No applicable suggestions to fix. Run Check spelling first.');
      return;
    }
    let cancelled = false;
    void (async () => {
      ...
      for (const issue of sorted) {
        ...
        const key = `${sectionKey}::${fieldPath}`;
        try {
          const text = getCvBuilderSectionFieldText(working, sectionKey, fieldPath);
          ...
          if (typeof result.text === 'string') {
            working = setCvBuilderSectionFieldText(working, sectionKey, fieldPath, result.text);
          } else if (result.alreadyApplied !== true) {
            const merged = text.slice(0, start) + suggestion + text.slice(end);
            working = setCvBuilderSectionFieldText(working, sectionKey, fieldPath, merged);
          }
          const mapped = result.issues.map((i) => ({
            ...i,
            sectionId: sectionKey,
  ...
  const formSectionTabs = useMemo(() => {
    ...
    if (data.projects.length > 0) base.push({ id: 'projects', label: 'Projects' });
    base.push({ id: 'achievements', label: 'Achievements' });
    if (data.certifications.length > 0) base.push({ id: 'certifications', label: 'Certifications' });
    if (data.languages.length > 0) base.push({ id: 'languages', label: 'Languages' });
  ...
  const [jumpSectionScrollEdges, setJumpSectionScrollEdges] = useState({ left: false, right: false });
  ...
  useEffect(() => {
    ...
    ro.observe(el);
    el.addEventListener('scroll', syncJumpSectionScrollEdges, { passive: true });
  ...
  const scrollJumpSectionStrip = useCallback((dir: -1 | 1) => {
    const el = jumpSectionScrollRef.current;
    if (!el) return;
    const delta = Math.min(220, Math.max(140, Math.floor(el.clientWidth * 0.65)));
  ...
  const jumpTabActive = useCallback(
    (sid: string) =>
      activeSection === sid ||
      (sid === 'experience' && typeof activeSection === 'string' && activeSection.startsWith('experience::')),
    [activeSection],
  );

  void portalFlip;
  const portalHost = dashboardMainRef?.current ?? null;
  const usePortaledExpandedEditor = mode === 'dashboard' && editorExpanded && portalHost !== null;
          ...
          ? showTripleChrome
            ...
            : usePortaledExpandedEditor
              ...
              : editorExpanded
                ...
                : 'max-h-none lg:max-h-[min(92dvh,calc(100dvh-13.5rem))]'
    ...
    >
      ...
      >
          ...
          <div className="mb-2 flex flex-col gap-2 pt-3">
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
                <h2 className="shrink-0 text-base font-bold text-white sm:text-[17px]">Edit your CV</h2>
                {saveStatus !== 'idle' ? (
                  <p className="text-[10px] text-white/40">
                    {saveStatus === 'saving' ? (
                      ...
                      <span className="text-rose-400/90">Save failed — keep editing or retry</span>
                    ) : null}
                  </p>
                ) : null}
              </div>
              <button
                type="button"
                title={editorExpanded ? 'Exit expanded editor' : 'Expand editor for more space'}
                aria-label={editorExpanded ? 'Exit expanded editor' : 'Expand editor for more space'}
                className={cn(
            ...
            <div className={cn('min-w-0', showTripleChrome && 'hidden')}>
              {dashboardTemplateExtras != null || dashboardTemplateMeta != null ? (
        ...
        {mode === 'dashboard' && !showTripleChrome ? (
          <details className="mb-2 group rounded-lg border border-[rgba(0,201,177,0.18)] bg-[rgba(0,201,177,0.06)] px-2.5 py-1.5">
            ...
            <p className="mt-2 border-t border-white/10 pt-2 text-[11px] leading-relaxed text-white/65">
              <span className="text-white">Full name</span> and <span className="text-white">email</span> are required. Add{' '}
              <span className="text-white">experience</span>, <span className="text-white">education</span>, and{' '}
              <span className="text-white">skills</span> before <span className="text-white">Generate summary</span> so AI has
              context.
            </p>
          </details>
        ) : mode !== 'dashboard' ? (
          <div className="mb-3 rounded-xl border border-[rgba(0,201,177,0.22)] bg-[rgba(0,201,177,0.07)] px-3 py-2.5 text-[11px] leading-relaxed text-white/70">
            <span className="font-semibold text-[#00C9B1]">What to fill in</span>
            {' — '}
            <span className="text-white">Full name</span> and <span className="text-white">email</span> are required to finish.
            Fill <span className="text-white">experience</span>, <span className="text-white">education</span>, and{' '}
            <span className="text-white">skills</span> before using <span className="text-white">Generate summary</span> (so AI has
        ...
        <div className="flex min-w-0 flex-col gap-1">
          <span className="shrink-0 text-[10px] font-semibold uppercase tracking-wider text-white/35">Jump to section</span>
              ...
              {formSectionTabs.map(({ id: sid, label }) => {
                ...
                return (
                  <button
                    key={sid}
                    type="button"
                    onClick={() => jumpToSection(sid, undefined, { scrollForm: true })}
                    className={cn(
                  ...
                  >
                    {done ? <Check className="h-3 w-3 shrink-0 text-[#22C55E]" strokeWidth={3} /> : null}
                    {label}
                  </button>
                );
              })}
      ...
      <div
        ...
        className={cn(
          ...
          showTripleChrome ? 'px-3 py-2' : 'overscroll-y-contain px-5 py-4 sm:px-6 sm:py-5',
        )}
      >
      <>
      <AccordionSection
      ...
      >
        <div className="grid gap-3">
          <Field
            ...
            onChange={(v) => update({ personal: { ...data.personal, name: v } })}
          ...
          <Field
            ...
            onChange={(v) => update({ personal: { ...data.personal, headline: v } })}
            onFocus={() => jumpToSection('personal')}
          />
          <Field
            label="Email *"
            ...
            onChange={(v) => update({ personal: { ...data.personal, email: v } })}
            onFocus={() => jumpToSection('personal')}
          />
          <Field
            label="Phone"
            value={data.personal.phone}
            onChange={(v) => update({ personal: { ...data.personal, phone: v } })}
            onFocus={() => jumpToSection('personal')}
          />
          <Field
            label="Location"
            ...
            onChange={(v) => update({ personal: { ...data.personal, location: v } })}
            onFocus={() => jumpToSection('personal')}
          />
          <button
            type="button"
          ...
          {optionalOpen ? (
            <div className="grid gap-3 border-t border-white/10 pt-3">
              <Field
                ...
                onChange={(v) => update({ personal: { ...data.personal, website: v } })}
                onFocus={() => jumpToSection('personal')}
              />
              <Field
                label="LinkedIn"
                ...
                onChange={(v) => update({ personal: { ...data.personal, linkedin: v } })}
                onFocus={() => jumpToSection('personal')}
              />
              <Field
                label="GitHub"
                ...
                onChange={(v) => update({ personal: { ...data.personal, github: v } })}
                onFocus={() => jumpToSection('personal')}
              />
              <Field
                ...
                onChange={(v) => update({ personal: { ...data.personal, portfolio: v } })}
                onFocus={() => jumpToSection('personal')}
              />
            </div>
          ) : null}
          {showIntlFields ? (
            <div className="grid gap-3 border-t border-white/10 pt-3">
              <p className="mb-2 rounded border border-[#00C9B1]/20 bg-[#00C9B1]/5 px-2 py-1 text-[10px] leading-snug text-[#00C9B1]/70">
                These fields are used by the <strong>{TEMPLATE_LABELS[template as CvTemplateId]}</strong> template. Fill in
                what applies to your region.
              </p>
              <Field
                label="Date of Birth"
                placeholder="e.g. 14 March 1990"
                value={data.personal.dateOfBirth ?? ''}
                onChange={(v) => update({ personal: { ...data.personal, dateOfBirth: v } })}
                onFocus={() => jumpToSection('personal')}
              />
              <Field
                ...
                onChange={(v) => update({ personal: { ...data.personal, placeOfBirth: v } })}
                onFocus={() => jumpToSection('personal')}
              />
              <Field
                label="Nationality"
                ...
                onChange={(v) => update({ personal: { ...data.personal, nationality: v } })}
                onFocus={() => jumpToSection('personal')}
              />
              <div>
                <label className="mb-1 block text-xs text-white/45">Gender</label>
                <select
                  className={fieldClass}
                  value={data.personal.gender ?? ''}
                  onChange={(e) => update({ personal: { ...data.personal, gender: e.target.value } })}
                  onFocus={() => jumpToSection('personal')}
                >
              ...
              <div>
                <label className="mb-1 block text-xs text-white/45">Marital Status</label>
                <select
                  className={fieldClass}
                  value={data.personal.maritalStatus ?? ''}
                  onChange={(e) => update({ personal: { ...data.personal, maritalStatus: e.target.value } })}
                  onFocus={() => jumpToSection('personal')}
                >
              ...
              <Field
                ...
                onChange={(v) => update({ personal: { ...data.personal, drivingLicence: v } })}
                onFocus={() => jumpToSection('personal')}
              />
            </div>
          ) : null}
          {showPhotoUpload ? (
            <div className="space-y-2 border-t border-white/10 pt-3">
              <label className="mb-1 block text-xs text-white/45">Profile Photo</label>
              {data.personal.photoUrl ? (
                <div className="relative h-20 w-16">
                  ...
                  <button
                    ...
                    onClick={() => update({ personal: { ...data.personal, photoUrl: '' } })}
                  >
                    ✕
                  </button>
                </div>
              ...
              <label className="flex w-fit cursor-pointer items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    void (async () => {
                      try {
                        const result = await compressImageFileToCvDataUrl(file);
                        update({ personal: { ...data.personal, photoUrl: result } });
                        setHeaderPreview({ showPhoto: true });
                      } catch (err) {
                        const msg = err instanceof Error ? err.message : CV_PHOTO_TOO_LARGE_USER_MESSAGE;
                        toast.error(msg);
                      }
                    })();
                  }}
              ...
              <p className="text-[10px] leading-snug text-white/30">
                JPG, PNG or WEBP · Large photos are resized and compressed before save · Used by French, German and Europass templates
              </p>
              <details className="mt-1">
                ...
                <p className="mb-1 mt-2 text-[10px] text-white/30">
                  Pasting a long <code className="text-white/45">data:image/…</code> string is compressed automatically so save does not fail.
                </p>
                <input
                  type="url"
                  ...
                  onChange={(e) => {
                    ...
                    void (async () => {
                      try {
                        ...
                        update({ personal: { ...data.personal, photoUrl: next } });
                        if (next.trim()) setHeaderPreview({ showPhoto: true });
                      } catch (err) {
                        const msg = err instanceof Error ? err.message : CV_PHOTO_TOO_LARGE_USER_MESSAGE;
                        toast.error(msg);
                      }
          ...
          {showHobbies ? (
            <div className="grid gap-2 border-t border-white/10 pt-3">
              <label className="mb-1 block text-xs text-white/45">Hobbies &amp; Interests</label>
              <textarea
                className={cn(fieldClass, 'min-h-[52px]')}
                rows={2}
                placeholder="e.g. Hiking, Photography, Open-source development"
                value={data.personal.hobbies ?? ''}
                onChange={(e) => update({ personal: { ...data.personal, hobbies: e.target.value } })}
                onFocus={() => jumpToSection('personal')}
              />
              <p className="text-[10px] leading-snug text-white/35">Used by French and German templates.</p>
            </div>
          ) : null}
          <button
            type="button"
            ...
            onClick={() =>
              update({
                personal: {
                  ...data.personal,
                  extras: [...data.personal.extras, { label: '', value: '' }],
                },
              })
            }
          >
          ...
          {data.personal.extras.length > 0 ? (
            <p className="text-[10px] leading-snug text-white/35">
              Use a short name for the link (left), then paste the URL or text (right) — e.g. &quot;Portfolio&quot; and
              https://yoursite.com
            </p>
          ) : null}
              ...
              <button
                ...
                onClick={() =>
                  update({
                    personal: {
                      ...data.personal,
                      extras: data.personal.extras.filter((_, j) => j !== i),
                    },
                  })
                }
              >
      ...
      >
        <div className="space-y-3">
          {experienceItems.length === 0 && uploadedCvHint ? (
            <div className="rounded-[10px] border border-[rgba(245,158,11,0.15)] bg-[rgba(245,158,11,0.06)] p-4">
              ...
              <p className="mb-2.5 text-xs leading-relaxed text-white/50">
                Your uploaded CV may have been too long for our parser to read completely.
                Try re-uploading your CV — we have improved our extraction.
              </p>
              {onRequestReparse ? (
                <button
                  type="button"
          ...
          {experienceItems.map((job, idx) => (
            <GlowCard key={job.id} className="border border-white/10" contentClassName="relative p-4">
              <div
              ...
              <div className="ml-6 grid gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <CvDateField
                    ...
                    helper={job.current ? 'Marked as current role — no end date.' : undefined}
                ...
                <label className="flex items-center gap-2 text-xs text-white/55">
                  <input
                    ...
                    onChange={(e) => {
                      const items = experienceItems.map((x) =>
                        x.id === job.id ? { ...x, current: e.target.checked, endDate: e.target.checked ? '' : x.endDate } : x,
                      );
                      update({ experience: { items } });
                    }}
                    onFocus={() => jumpToSection('experience', job.id)}
                ...
                <div>
                  <label className="mb-1 block text-xs text-white/45">What you achieved in this role</label>
                  <p className="mb-1.5 text-[10px] leading-snug text-white/35">
                    Put each achievement on its own line. You don&apos;t need • or dashes — we format those on your CV.
                  </p>
                  <textarea
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      const line = job.bullets.find((b) => b.trim()) || job.title;
                      const out = await runAiBullets(job.id, line);
                      if (out && !profileId?.trim()) {
                        const items = experienceItems.map((x) =>
                          x.id === job.id ? { ...x, bullets: [...x.bullets, out] } : x,
                        );
                        update({ experience: { items } });
                      }
                    }}
          ...
          {educationItems.length === 0 && uploadedCvHint ? (
            <div className="rounded-[10px] border border-[rgba(245,158,11,0.15)] bg-[rgba(245,158,11,0.06)] p-4">
              ...
              <p className="text-xs leading-relaxed text-white/50">
                Add your education manually, or re-upload your CV to try extraction again.
              </p>
            </div>
          ) : null}
          {educationItems.map((ed) => (
            <GlowCard key={ed.id} className="border border-white/10" contentClassName="relative p-4">
              <button
                type="button"
                className="absolute right-2 top-2 text-white/35 hover:text-white"
              ...
              <div className="grid gap-2 pr-6">
                <Field
                  ...
                  value={ed.degree}
                  onChange={(v) => {
                    const items = educationItems.map((x) => (x.id === ed.id ? { ...x, degree: v } : x));
                ...
                <Field
                  ...
                  value={ed.field}
                  onChange={(v) => {
                    const items = educationItems.map((x) => (x.id === ed.id ? { ...x, field: v } : x));
                    update({ education: { items } });
                  }}
                  onFocus={() => jumpToSection('education')}
                />
                <Field
                  ...
                  value={ed.school}
                  onChange={(v) => {
                    const items = educationItems.map((x) => (x.id === ed.id ? { ...x, school: v } : x));
                    update({ education: { items } });
                  }}
                  onFocus={() => jumpToSection('education')}
                />
                ...
                <Field
                  ...
                  value={ed.grade ?? ''}
                  onChange={(v) => {
                    const items = educationItems.map((x) => (x.id === ed.id ? { ...x, grade: v } : x));
                    update({ education: { items } });
                  }}
                  onFocus={() => jumpToSection('education')}
                />
        ...
        right={
          <button
            ...
            onClick={() =>
              update({
                skills: {
                  categories: [...skillCategories, { id: newLocalId(), name: '', skills: [] }],
                },
              })
            }
          >
      ...
      >
        <div className="space-y-4">
          {skillCategories.every((c) => (Array.isArray(c.skills) ? c.skills : []).length === 0) && uploadedCvHint ? (
            <div className="rounded-[10px] border border-[rgba(245,158,11,0.15)] bg-[rgba(245,158,11,0.06)] p-4">
              <p className="mb-1 text-[13px] font-semibold text-[#F59E0B]">
                Skills not extracted from your CV
              </p>
              <p className="text-xs leading-relaxed text-white/50">
                Type skills below and press Enter to add them, or re-upload your CV to try extraction again.
              </p>
            </div>
          ) : null}
          {skillCategories.map((cat, ci) => (
            <div key={cat.id} className="rounded-xl border border-white/10 bg-[#111616] p-3">
              <div className="mb-2 flex gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    update({
                      skills: {
                        categories: skillCategories.filter((_, i) => i !== ci),
                      },
                    })
                  }
                >
      ...
      >
        {uploadedCvHint && normalizeText(data.summary?.text as unknown).trim() === '' ? (
          <div
            className="mb-3 rounded-lg border border-amber-500/20 px-3 py-2.5"
            style={{ background: 'rgba(245,158,11,0.08)' }}
          >
            <p className="mb-1 text-xs font-medium text-[#F59E0B]">Your uploaded CV had no summary</p>
            <p className="mb-3 text-[11px] leading-relaxed text-white/50">
              A professional summary is recommended if you have 3+ years of experience. Use the AI button to generate one, or hide
        ...
        <p className="mb-3 text-[11px] leading-relaxed text-white/40">
          Add this <span className="text-white/55">after</span> your title, jobs, and skills so AI can match your story. Use &quot;Generate with AI&quot; for a first draft, then edit in your own voice.
        </p>
      ...
      >
        <p className="mb-3 text-[11px] leading-relaxed text-white/40">
          Optional — showcase products, apps, or significant work that doesn&apos;t fit under a single employer.
        </p>
        {data.projects.map((p) => (
          <GlowCard key={p.id} className="mb-3 border border-white/10" contentClassName="p-4">
            <p className="mb-2 text-sm font-semibold text-white">Project</p>
            <Field
              label="Project name"
              placeholder="e.g. ApplyMate job tracker"
              value={p.name}
              onChange={(v) => update({ projects: data.projects.map((x) => (x.id === p.id ? { ...x, name: v } : x)) })}
              onFocus={() => jumpToSection('projects')}
            />
            <label className="mb-1 mt-2 block text-xs text-white/45">What you built</label>
            <p className="mb-1 text-[10px] text-white/30">Describe the project in plain language — what problem it solved or who uses it.</p>
            <BuilderRichTextField
              placeholder="e.g. A web app that helps job seekers track applications and tailor CVs with AI hints."
              value={p.description}
              onChange={(description) =>
                update({ projects: data.projects.map((x) => (x.id === p.id ? { ...x, description } : x)) })
              }
              onFocus={() => jumpToSection('projects')}
            />
            <div className="mt-2">
              <TagInput
                ...
                onChange={(technologies) =>
                  update({ projects: data.projects.map((x) => (x.id === p.id ? { ...x, technologies } : x)) })
                }
                onFocus={() => jumpToSection('projects')}
              />
            </div>
            <Field
                ...
                update({
                  projects: data.projects.map((x) => {
                    ...
                    if (!trimmed && description) {
                      ...
                      if (prevUrl) {
                        const esc = prevUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                        description = description
                          .replace(new RegExp(`<a[^>]*href=["']?${esc}["']?[^>]*>.*?<\\/a>`, 'gi'), '')
                          .replace(prevUrl, '')
                          .trim();
                      }
                    }
              ...
              onFocus={() => jumpToSection('projects')}
            />
            <label className="mb-1 mt-2 block text-xs text-white/45">Highlights (optional)</label>
            <p className="mb-1 text-[10px] text-white/30">One point per line — no bullet symbols needed.</p>
            <BuilderRichTextField
              minHeightClass="min-h-[64px]"
              placeholder="e.g. Shipped MVP in 6 weeks — one highlight per line"
              value={p.bullets}
              onChange={(bullets) =>
                update({ projects: data.projects.map((x) => (x.id === p.id ? { ...x, bullets } : x)) })
              }
              onFocus={() => jumpToSection('projects')}
            />
          </GlowCard>
      ...
      >
        <p className="mb-3 text-[11px] leading-relaxed text-white/40">
          Awards, recognitions, or notable credentials (title, issuer, date, optional notes).
        </p>
        {data.achievements.map((a: CVBuilderAchievement) => (
          <GlowCard key={a.id} className="mb-3 last:mb-0" contentClassName="p-4">
            <p className="mb-2 text-sm font-semibold text-white">Achievement</p>
            <Field
              label="Title"
              placeholder="e.g. AWS Certified DevOps Engineer"
              value={a.title}
              onChange={(v) =>
                update({ achievements: data.achievements.map((x) => (x.id === a.id ? { ...x, title: v } : x)) })
              }
            ...
            <Field
              ...
              value={a.issuer}
              onChange={(v) =>
                update({ achievements: data.achievements.map((x) => (x.id === a.id ? { ...x, issuer: v } : x)) })
              }
              onFocus={() => jumpToSection('achievements')}
            />
            <CvDateField
              ...
              value={a.date}
              onChange={(v) =>
                update({ achievements: data.achievements.map((x) => (x.id === a.id ? { ...x, date: v } : x)) })
              }
              onFocus={() => jumpToSection('achievements')}
            />
            <label className="mb-1 mt-2 block text-xs text-white/45">Detail / note (optional)</label>
            <textarea
              ...
              onChange={(e) =>
                update({
                  achievements: data.achievements.map((x) => (x.id === a.id ? { ...x, detail: e.target.value } : x)),
                })
              }
              onFocus={() => jumpToSection('achievements')}
            />
            <button
              ...
              onClick={() => update({ achievements: data.achievements.filter((x) => x.id !== a.id) })}
            >
              ✕ Remove
            </button>
          </GlowCard>
      ...
      >
        <p className="mb-3 text-[11px] leading-relaxed text-white/40">
          Professional certificates, licences, or exam passes (name, issuer, date).
        </p>
        {data.certifications.map((c) => (
          <GlowCard key={c.id} className="mb-3 last:mb-0" contentClassName="p-4">
            <p className="mb-2 text-sm font-semibold text-white">Certification</p>
            <Field
              label="Name"
              value={c.name}
              onChange={(v) => update({ certifications: data.certifications.map((x) => (x.id === c.id ? { ...x, name: v } : x)) })}
              onFocus={() => jumpToSection('certifications')}
            />
            <Field
              label="Issuing organisation"
              value={c.issuer}
              onChange={(v) => update({ certifications: data.certifications.map((x) => (x.id === c.id ? { ...x, issuer: v } : x)) })}
              onFocus={() => jumpToSection('certifications')}
            />
            <div className="mt-2">
              <CvDateField
                ...
                preferYear
                value={c.date}
                onChange={(v) => update({ certifications: data.certifications.map((x) => (x.id === c.id ? { ...x, date: v } : x)) })}
                onFocus={() => jumpToSection('certifications')}
              />
            </div>
            <Field
              ...
              placeholder="https://…"
              value={c.url}
              onChange={(v) => update({ certifications: data.certifications.map((x) => (x.id === c.id ? { ...x, url: v } : x)) })}
              onFocus={() => jumpToSection('certifications')}
            />
          </GlowCard>
        ))}
      ...
      <AccordionSection
        ...
        right={
          <button
            ...
            onClick={() =>
              update({
                languages: [...data.languages, { id: newLocalId(), language: '', proficiency: 'Fluent' }],
              })
            }
          >
            + Add
      ...
      >
        <p className="mb-3 text-[11px] leading-relaxed text-white/40">List languages you can use professionally and your level.</p>
        {data.languages.map((l) => {
          const cefrExpanded = showCefr && langCefrOpen[l.id] === true;
          const setCefrField = (field: keyof CVBuilderLanguage, value: string) =>
            update((d) => ({
              ...d,
              languages: d.languages.map((x) =>
                x.id === l.id ? { ...x, [field]: value ? value : undefined } : x,
              ),
            }));
          return (
            <GlowCard key={l.id} className="mb-3 last:mb-0" contentClassName="p-4">
              <div className="grid grid-cols-2 gap-2">
                <Field
                  label="Language"
                  value={l.language}
                  onChange={(v) => update({ languages: data.languages.map((x) => (x.id === l.id ? { ...x, language: v } : x)) })}
                  onFocus={() => jumpToSection('languages')}
                />
                <div>
                  <label className="mb-1 block text-xs text-white/45">Proficiency</label>
                  <select
                    className={fieldClass}
                    value={l.proficiency}
                    onChange={(e) =>
                      update({
                        languages: data.languages.map((x) =>
                          x.id === l.id ? { ...x, proficiency: e.target.value as typeof l.proficiency } : x,
                        ),
                      })
                    }
                    onFocus={() => jumpToSection('languages')}
                  >
                    {(['Native', 'Fluent', 'Professional', 'Intermediate', 'Basic'] as const).map((o) => (
              ...
              {showCefr ? (
                <div>
                  <button
                    ...
                    onClick={() => setLangCefrOpen((m) => ({ ...m, [l.id]: !cefrExpanded }))}
                  >
                    {cefrExpanded ? 'Hide CEFR breakdown' : '＋ Add CEFR breakdown'}
                  </button>
                  {cefrExpanded ? (
                    <div className="mt-3">
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                        <div>
                          <label className="mb-1 block text-[10px] text-white/45">Listening</label>
                          <select
                            className={fieldClass}
                            value={l.listening ?? ''}
                            onChange={(e) => setCefrField('listening', e.target.value)}
                            onFocus={() => jumpToSection('languages')}
                          >
                        ...
                        <div>
                          <label className="mb-1 block text-[10px] text-white/45">Reading</label>
                          <select
                            className={fieldClass}
                            value={l.reading ?? ''}
                            onChange={(e) => setCefrField('reading', e.target.value)}
                            onFocus={() => jumpToSection('languages')}
                          >
                            {CEFR_LEVEL_OPTIONS.map((o) => (
                        ...
                        <div>
                          <label className="mb-1 block text-[10px] text-white/45">Spoken interaction</label>
                          <select
                            className={fieldClass}
                            value={l.spokenInteraction ?? ''}
                            onChange={(e) => setCefrField('spokenInteraction', e.target.value)}
                            onFocus={() => jumpToSection('languages')}
                          >
                            {CEFR_LEVEL_OPTIONS.map((o) => (
                        ...
                        <div>
                          <label className="mb-1 block text-[10px] text-white/45">Spoken production</label>
                          <select
                            className={fieldClass}
                            value={l.spokenProduction ?? ''}
                            onChange={(e) => setCefrField('spokenProduction', e.target.value)}
                            onFocus={() => jumpToSection('languages')}
                          >
                            {CEFR_LEVEL_OPTIONS.map((o) => (
                        ...
                        <div>
                          <label className="mb-1 block text-[10px] text-white/45">Writing</label>
                          <select
                            className={fieldClass}
                            value={l.writing ?? ''}
                            onChange={(e) => setCefrField('writing', e.target.value)}
                            onFocus={() => jumpToSection('languages')}
                          >
                            {CEFR_LEVEL_OPTIONS.map((o) => (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  update({ languages: data.languages.filter((x) => x.id !== l.id) });
                }}
              >
                ✕ Remove language
              </button>
      ...
      {showReferences ? (
        ...
        >
          <p className="mb-2 text-[10px] leading-snug text-white/40">
            References are displayed on UK CVs. For other templates, write &apos;References available upon request.&apos; in your
            summary or leave blank.
          </p>
          {data.references.map((ref) => (
            <GlowCard key={ref.id} className="mb-3 last:mb-0" contentClassName="p-4">
              <Field
                label="Full Name"
                value={ref.name}
                onChange={(v) =>
                  update((d) => ({
                    ...d,
                    references: d.references.map((r) => (r.id === ref.id ? { ...r, name: v } : r)),
                  }))
                }
              ...
              <Field
                ...
                onChange={(v) =>
                  update((d) => ({
                    ...d,
                    references: d.references.map((r) => (r.id === ref.id ? { ...r, title: v } : r)),
                  }))
                }
                onFocus={() => jumpToSection('references')}
              />
              <Field
                ...
                onChange={(v) =>
                  update((d) => ({
                    ...d,
                    references: d.references.map((r) => (r.id === ref.id ? { ...r, company: v } : r)),
                  }))
                }
                onFocus={() => jumpToSection('references')}
              />
              <Field
                ...
                onChange={(v) =>
                  update((d) => ({
                    ...d,
                    references: d.references.map((r) => (r.id === ref.id ? { ...r, email: v } : r)),
                  }))
                }
                onFocus={() => jumpToSection('references')}
              />
              <Field
                ...
                onChange={(v) =>
                  update((d) => ({
                    ...d,
                    references: d.references.map((r) => (r.id === ref.id ? { ...r, phone: v } : r)),
                  }))
                }
                onFocus={() => jumpToSection('references')}
              />
      ...
      {data.parsedCustomSections.map((block) => {
        ...
        return (
          <AccordionSection
            key={block.sectionId}
            id={sid}
            title={(block.title || 'Custom section').trim().slice(0, 72) || 'Custom section'}
          ...
          >
            ...
            <Field
              ...
              onChange={(v) =>
                update({
                  parsedCustomSections: data.parsedCustomSections.map((b) =>
                    b.sectionId === block.sectionId ? { ...b, title: v } : b,
                  ),
                })
              }
            ...
            {block.items.map((item: CVBuilderParsedCustomItem) => (
              <GlowCard key={item.id} className="mb-3 last:mb-0" contentClassName="p-4">
                <Field
                  label="Text"
                ...
                <Field
                  ...
                  onFocus={() => jumpToSection(sid)}
                />
                <label className="mb-1 mt-2 block text-xs text-white/45">Sub-items (optional, one per line)</label>
                <textarea
                    ...
                    update({
                      parsedCustomSections: data.parsedCustomSections.map((b) =>
                        b.sectionId === block.sectionId
                          ? {
                              ...b,
                              items: b.items.map((it) => (it.id === item.id ? { ...it, subItems } : it)),
                            }
                          : b,
                      ),
                    });
      ...
      >
        <p className="mb-3 text-[11px] leading-relaxed text-white/40">
          Extra titled blocks (title + body). Use when you want a simple section not tied to upload parsing.
        </p>
        {data.customSections.map((c) => (
          <GlowCard key={c.id} className="mb-3 last:mb-0" contentClassName="p-4">
            <Field
              ...
              onChange={(v) =>
                update({ customSections: data.customSections.map((x) => (x.id === c.id ? { ...x, title: v } : x)) })
              }
              onFocus={() => jumpToSection('custom-legacy')}
            />
            <label className="mb-1 mt-2 block text-xs text-white/45">Content</label>
            <textarea
              className={cn(fieldClass, 'min-h-[100px]')}
              value={normalizeText(c.body as unknown)}
              onChange={(e) =>
                update({
                  customSections: data.customSections.map((x) => (x.id === c.id ? { ...x, body: e.target.value } : x)),
                })
              }
              onFocus={() => jumpToSection('custom-legacy')}
            />
            <button
              type="button"
              className="mt-2 text-xs text-white/45 hover:text-[#EF4444]"
              onClick={() => update({ customSections: data.customSections.filter((x) => x.id !== c.id) })}
            >
              ✕ Remove section
            </button>
          </GlowCard>
  ...
  const previewDoc = useMemo(
    () => (
      <CVDocumentPreview
        ...
        optionalSectionPresence={mode === 'dashboard' ? optionalSectionPresence : undefined}
        sectionOrder={mode === 'dashboard' ? previewSectionOrder : undefined}
        onReorderSections={mode === 'dashboard' ? handlePreviewReorderSections : undefined}
      />
    ),
    [
      data,
    ...
    () => (
      <div
        ...
        className={cn(
          ...
          showTripleShell ? 'max-w-full' : 'max-w-[min(100%,820px)] max-lg:max-w-full',
        )}
      ...
      >
        <div className={cn(mode === 'dashboard' ? '' : 'origin-top scale-[0.95]')}>
          {mode === 'dashboard' ? <CVEditProvider value={cvEditContextValue}>{previewDoc}</CVEditProvider> : previewDoc}
        </div>
      </div>
    ),
      ...
      {mode === 'onboarding' ? (
        <div className="sticky top-0 z-10 shrink-0 border-b border-white/10 bg-[#080A0A]/95 px-4 py-3 backdrop-blur-md lg:px-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-white/45">Preview</p>
            {tplPills}
          </div>
        </div>
      ) : null}
  ...
  const builderTree = (
    <div
      className={cn(
        ...
        mode === 'dashboard' && showTripleShell && 'min-h-0 flex-1 overflow-hidden',
      )}
    >
      {!showTripleShell ? (
      <div
        ref={splitRowRef}
        style={{ ['--cv-editor-width' as string]: `${editorWidthPct}%` } as CSSProperties}
        className={cn(
      ...
      >
        ...
        >
          <button
            ...
            className={cn(
              ...
              mobileTab === 'edit' ? 'bg-[#00C9B1]/15 text-[#00C9B1]' : 'text-white/45',
            )}
          ...
          <button
            ...
            className={cn(
              'flex-1 rounded-lg py-2 text-sm font-semibold',
              mobileTab === 'preview' ? 'bg-[#00C9B1]/15 text-[#00C9B1]' : 'text-white/45',
            )}
        ...
        <div
          className={cn(
            ...
            mode === 'onboarding' && (mobileTab === 'edit' ? 'block' : 'hidden lg:flex'),
            'w-full lg:w-[var(--cv-editor-width)] lg:min-w-[16rem] lg:max-w-[62%] lg:shrink-0 lg:grow-0',
            mode === 'dashboard' && editorExpanded && usePortaledExpandedEditor && 'hidden',
            mode === 'dashboard' &&
              editorExpanded &&
              !usePortaledExpandedEditor &&
        ...
        >
          {!(mode === 'dashboard' && editorExpanded && usePortaledExpandedEditor) ? leftPanel : null}
        </div>
        ...
        <div
          className={cn(
            ...
            mode === 'dashboard' ? 'min-h-[min(55dvh,480px)] lg:min-h-0' : '',
            mode === 'dashboard' ? 'block' : mobileTab === 'preview' ? 'block' : 'hidden lg:flex',
            mode === 'dashboard' && editorExpanded && 'lg:hidden',
          )}
        >
          {previewPanel}
        ...
        <div
          ...
          className="mt-0 flex min-h-0 min-w-0 w-full flex-1 flex-col overflow-hidden rounded-2xl border border-white/[0.06]"
        >
          <div className="flex min-h-0 min-w-0 w-full flex-1 flex-row overflow-hidden">
            ...
            <div
              ...
              onPointerDown={tripleColumn.rightCollapsed ? undefined : tripleColumn.onRightResizePointerDown}
              className={cn(
                'w-1 shrink-0 cursor-col-resize bg-white/[0.04] transition-colors hover:bg-[#00C9B1]/40',
                tripleColumn.rightCollapsed && 'pointer-events-none opacity-40',
              )}
            />
            <div
              style={{
                flex: tripleColumn.rightCollapsed ? '0 0 0px' : `0 0 ${tripleColumn.rightPct}%`,
              }}
              className={cn(
                'flex min-h-0 min-w-0 shrink-0 flex-col overflow-hidden border-l border-white/[0.07] bg-[#0C0F0F] transition-[flex,opacity] duration-300 ease-out',
                !tripleColumn.rightCollapsed && 'min-w-[300px] xl:min-w-[320px]',
                tripleColumn.rightCollapsed ? 'border-l-0 opacity-0' : 'opacity-100',
              )}
            >
              {!tripleColumn.rightCollapsed ? (tripleColumnRightSlot ?? tripleColumn.rightSlot ?? null) : null}
            </div>
          </div>
        </div>
      ) : null}

      {mode === 'dashboard' && editorExpanded && usePortaledExpandedEditor && portalHost
        ? createPortal(
            <>
              <button
                type="button"
      ...
      {mode === 'onboarding' ? (
        <div className="mt-6 flex w-full flex-col gap-3 border-t border-white/10 pt-4 sm:flex-row sm:items-center sm:justify-between">
          <button type="button" className="text-sm text-white/45 hover:text-white" onClick={() => void onSkip?.()}>
            Skip for now →
          </button>
          <Button
          ...
          <p className="text-[11px] text-white/25">{filled} sections with content</p>
        </div>
      ) : null}
  ...
  if (isTailorView) {
    return <CvOverlayLayerProvider zIndex={TAILOR_CV_PORTAL_Z}>{builderTree}</CvOverlayLayerProvider>;
  }
  return builderTree;
}
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1 rounded-lg border border-[rgba(255,255,255,0.10)] bg-[#0c1010] p-2">
        {skills.map((s, i) => (
          ...
          >
            {s}
            <button type="button" className="text-white/50 hover:text-white" onClick={() => onChange(skills.filter((x) => x !== s))}>
              ✕
            </button>
          </span>
        ))}
    ...
    <div id={`cv-section-${id}`} className="scroll-mt-20">
      <GlowCard className="mb-3 hover:!translate-y-0" contentClassName="p-0">
        <div className="flex w-full items-center gap-1 px-4 py-3">
          ...
          >
            <ChevronDown className={cn('h-4 w-4 shrink-0 transition', expanded ? 'rotate-0' : '-rotate-90')} />
            <span
              className={cn('text-sm font-semibold text-white', sectionHiddenStyle && 'line-through decoration-white/35')}
            >
              {title}
            </span>
            {sectionHiddenStyle ? (
          ...
          {visibilityToggle ? (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                padding: 4,
                color: visibilityToggle.visible ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.15)',
              }}
            >
        ...
        {expanded ? (
          <div className="border-t border-white/10 px-4 pb-4 pt-2" onFocusCapture={onFocusSection}>
            {children}
          </div>
        ) : null}
      </GlowCard>
