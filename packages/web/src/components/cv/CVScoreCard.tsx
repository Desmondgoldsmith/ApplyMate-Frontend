...
const EMPTY_SECTION = (): SectionScore => ({ score: 0, weight: 0, feedback: '', flags: [] });

function parseSectionScore(raw: unknown): SectionScore {
  if (raw !== null && typeof raw === 'object' && !Array.isArray(raw) && 'score' in (raw as object)) {
    const o = raw as Record<string, unknown>;
    const sc = Number(o.score);
    const w = o.weight !== undefined ? Number(o.weight) : NaN;
    const flags: CVFlag[] = Array.isArray(o.flags)
      ? (o.flags as unknown[]).filter((f): f is CVFlag => f !== null && typeof f === 'object')
      : [];
    return {
...
function parseAts(raw: unknown): ATSCompatibility {
  if (raw !== null && typeof raw === 'object' && !Array.isArray(raw)) {
    ...
    const passed = Array.isArray(o.passed) ? o.passed.filter((x): x is string => typeof x === 'string') : [];
...
function parseJobMatchSection(raw: unknown): JobMatchSectionScore | undefined {
  ...
  if (typeof raw !== 'object' || Array.isArray(raw)) {
    return { ...base, missingSkills: [], alignmentInsights: [], recommendations: [] };
  }
  const o = raw as Record<string, unknown>;
  return {
    ...base,
    missingSkills: parseStringArray(o.missingSkills ?? o.missing_skills),
    alignmentInsights: parseStringArray(o.alignmentInsights ?? o.alignment_insights),
...
function parseImprovementsFromBreakdown(raw: unknown): CVImprovement[] {
  ...
  for (const x of raw) {
    ...
    if (o.priority === 1 || o.priority === 2 || o.priority === 3) priority = o.priority;
...
function isNewShapeBreakdown(b: Record<string, unknown> | undefined): boolean {
  const s = b?.sections;
  return s !== null && typeof s === 'object' && !Array.isArray(s) && 'experience' in (s as object);
}
...
function normalizeScoreBreakdown(
  ...
  if (isNewShapeBreakdown(breakdown)) {
    ...
    const st = typeof breakdown!.strengths === 'object' && Array.isArray(breakdown!.strengths)
      ? (breakdown!.strengths as string[]).filter((x) => typeof x === 'string')
      : [];
    const flags: CVFlag[] = Array.isArray(breakdown!.flags)
      ? (breakdown!.flags as unknown[]).filter((f): f is CVFlag => f !== null && typeof f === 'object')
      : [];
    const cs = breakdown!.careerStage;
    const careerStage: CareerStage =
      cs === 'student' || cs === 'early' || cs === 'mid' || cs === 'senior' ? cs : 'student';
    const overallN = breakdown!.overall !== undefined ? Number(breakdown!.overall) : ringScore;
    const jmRaw = s.jobMatch ?? s.job_match;
    const jobMatch = jmRaw !== undefined ? parseJobMatchSection(jmRaw) : undefined;
    return {
...
export function CVScoreCard({
  ...
  const raw = breakdown !== null && typeof breakdown === 'object' ? (breakdown as Record<string, unknown>) : undefined;
  ...
  const strengthsDisplay = br.strengths.length > 0 ? br.strengths : strengths ?? [];

  const sectionOrder: { key: keyof CVScoreBreakdown['sections']; label: string }[] = [
  ...
  const jobMatchFlags = (br.flags ?? []).filter((f) => (f.section ?? '').toLowerCase() === 'jobmatch');
  const jobMatchFromFlags = {
    missing: jobMatchFlags.filter((f) => /skill|keyword|missing/i.test(f.message)).map((f) => f.message),
    insights: jobMatchFlags.filter((f) => /align|fit|overlap|match/i.test(f.message)).map((f) => f.message),
    recs: jobMatchFlags.map((f) => f.fix).filter((x) => typeof x === 'string' && x.trim().length > 0) as string[],
  };
  ...
  const showJobMatchPanel =
    Boolean(jobMatch && (jobMatch.weight > 0 || jobMatch.score > 0 || missingSkillsDisplay.length > 0));
  const showJobMatchSkippedNote = Boolean(
    jobMatch &&
      jobMatch.weight === 0 &&
      (jobMatch.feedback?.trim() || (jobMatch.flags?.length ?? 0) > 0),
  );
  ...
  const atsCompatibilitySection = (compactLayout: boolean) => (
    ...
    >
      <div className={cn('flex flex-col gap-1', compactLayout ? 'mb-2' : 'mb-3')}>
        <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1.5">
          <div className="flex min-w-0 max-w-[min(100%,14rem)] items-center gap-1 sm:max-w-none">
            ...
            <InfoHint
              text={[br.ats.methodologyNote?.trim(), atsHeuristicDisclaimer].filter(Boolean).join('\n\n')}
      ...
      {br.ats.passed.map((check, i) => (
        <div key={`p-${i}`} className={cn('flex items-start gap-2', compactLayout ? 'mb-1.5' : 'mb-2')}>
          <span className="mt-0.5 shrink-0 text-xs text-green-400" title={atsPassedTooltip}>
            ✓
          </span>
          <span
            className={cn(
  ...
  return (
    <GlowCard
      className={cn('border border-[rgba(0,201,177,0.15)]', 'overflow-x-visible')}
      contentClassName={cn(
    ...
    >
      <div className={cn('flex flex-col', mode === 'compact' ? 'gap-3' : 'gap-5')}>
        <div className="flex shrink-0 flex-col items-center px-2 pb-2 pt-0.5 sm:px-3">
          ...
          >
            ...
            <circle
              ...
              strokeDashoffset={RING_C * (1 - Math.min(100, Math.max(0, score)) / 100)}
              strokeLinecap="round"
          ...
          {scorePreview && !hideJobMatch ? (
            <span className="mt-2 max-w-[14rem] text-center text-[10px] font-semibold leading-snug text-amber-200/90">
              ...
              <span className="mt-1 block font-normal text-white/40">
                Stored CV score updates only when you run a full scan without pasted job text.
              </span>
            </span>
          ) : null}
        </div>
        ...
        <div className="min-w-0 flex-1 overflow-x-visible">
          ...
          <div className="overflow-x-visible">
            {sectionOrder.map(({ key, label }) => {
              ...
              const weightPct = w !== undefined && w > 0 ? Math.round(w * 100) : null;
              return (
                <div
                  key={key}
                ...
                >
                  <div className="flex w-full min-w-0 items-center gap-1.5 text-[10px] leading-tight sm:gap-2 sm:text-[11px]">
                    ...
                    {weightPct != null ? (
                      ...
                      <span className="shrink-0 whitespace-nowrap text-[rgba(255,255,255,0.22)]">—</span>
                    )}
                    <span
          ...
          {!hideJobMatch && showJobMatchPanel ? (
            <div className="mt-2 border-t border-white/[0.07] px-4 pb-1 pt-3">
              <p className="text-[10px] font-medium uppercase tracking-[0.08em] text-white/30">Job match</p>
              <p className="mt-1 text-xl font-bold tabular-nums text-[#00C9B1]">{Math.round(jobMatch?.score ?? 0)}%</p>
              <p className="mt-1 text-[11px] leading-relaxed text-white/35">
                Heuristic alignment with the job text you provided — not a hiring decision or interview guarantee.
              </p>
              {jobMatch?.feedback?.trim() ? (
                <p className="mb-3 text-xs leading-relaxed text-white/65">{jobMatch.feedback.trim()}</p>
              ) : null}
          ...
          {!hideJobMatch && mode === 'full' && showJobMatchSkippedNote && !showJobMatchPanel ? (
          ...
          {br.overall <= 20 ? (
            ...
            >
              <p style={{ fontSize: 12, color: '#F59E0B', marginBottom: 4, fontWeight: 600 }}>
                Score may not reflect your actual CV
              </p>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', lineHeight: 1.5 }}>
                If you uploaded a CV, some sections may not have been extracted correctly.
                Re-upload your CV from the CV editor page to get an accurate score.
              </p>
            </div>
          ) : null}
          ...
          {mode === 'compact' && improvementsCount != null && improvementsCount > 0 ? (
            <p className="mt-4 text-[13px] text-white/45">
              {improvementsCount} improvement{improvementsCount === 1 ? '' : 's'} available →
            </p>
          ) : null}
        </div>
      </div>
      ...
      {mode === 'full' && strengthsDisplay.length > 0 ? (
        <div className="mt-6 border-t border-white/[0.08] pt-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#22C55E]/90">Strengths</p>
        ...
        <div className="mt-4 flex flex-col gap-2">
          {flags.map((f, i) => (
            <p
              ...
              className={cn(
                'text-xs',
                f.type === 'error' ? 'text-[#EF4444]' : f.type === 'warning' ? 'text-[#F59E0B]' : 'text-white/45',
              )}
            >
              {f.text}
            </p>
