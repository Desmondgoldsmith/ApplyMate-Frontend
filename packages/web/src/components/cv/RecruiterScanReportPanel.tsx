...
export function RecruiterScanReportPanel({
  ...
  const copyShare = async () => {
    const text = buildRecruiterScanShareSnippet(session);
    try {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[126] flex justify-end bg-black/45 p-0 sm:p-3"
      ...
      >
        <motion.aside
          ...
          exit={{ opacity: 0, x: 16 }}
          className="pointer-events-auto flex h-full w-full max-w-md flex-col border-white/[0.08] bg-[#0C0F0F] shadow-[0_20px_48px_rgba(0,0,0,0.55)] sm:max-h-[calc(100vh-1.5rem)] sm:rounded-2xl sm:border"
        ...
        >
          ...
          <div className="border-b border-white/[0.06] px-4 py-3">
            ...
            <p className="mt-2 text-[10px] text-white/40">
              Orange glow on your CV preview shows where attention landed (hover sections for notes).
            </p>
          </div>
