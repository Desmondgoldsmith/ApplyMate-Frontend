...
const STORAGE_COMPLETED_TAILOR_KEY =
...
function minDescriptionCharsForAnalyze(
 ...
 */
function mergeJobAnalysisForApply(
  ...
  return (
    <>
      <motion.div
        ...
        className="flex w-full min-w-0 max-w-full flex-col space-y-4 overflow-x-hidden lg:min-h-[calc(100dvh-7.5rem)]"
      >
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          className="grid w-full min-w-0 flex-1 min-h-0 gap-4 max-lg:flex max-lg:flex-col lg:grid-cols-[minmax(0,44%)_minmax(0,1fr)] lg:items-stretch lg:gap-6"
        >
          <div className="flex min-h-0 flex-col max-lg:shrink-0 lg:max-h-[calc(100dvh-7.5rem)] lg:overflow-y-auto lg:pr-1 app-scrollbar">
            <div data-tour="analyzer-form" className="min-h-0 shrink-0">
              ...
              >
                {cvProfiles.length > 1 ? (
                  <div className="mb-6 rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-4 sm:px-5">
                      ...
                      >
                        {cvProfiles.map((p) => (
                          <option
                            key={p.id}
                            value={p.id}
                            className="bg-[#141f21] text-white"
                          >
                            {p.name}
                          </option>
                        ))}
                    ...
                    <p className="mt-2 truncate text-[12px] text-white/35">
                      Job fit uses your structured CV. CV quality is separate
                      from role match %.
                      {selectedProfile?.isDefault ? (
                        <span className="ml-1 font-medium text-[#00C9B1]">
                          Default CV
                        </span>
                      ) : null}
                    </p>
                ...
                <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <h2 className="text-base font-semibold text-white sm:text-[16px]">
                    Analyze a Job
                  </h2>
                  <button
                    type="button"
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <Button
                    ...
                    className="hidden min-h-[52px] rounded-[10px] bg-gradient-to-br from-[#00C9B1] to-[#00A896] text-[15px] font-semibold text-[#080A0A] shadow-[0_6px_24px_rgba(0,201,177,0.35)] transition hover:brightness-105 active:scale-[0.99] lg:flex"
                  >
                  ...
                  <p className="text-center text-[11px] leading-relaxed text-white/40">
                    Runs match scoring and your AI recruiter report (when quota
                    allows). The report opens collapsed below with a glowing
                    header.
                  </p>
                </div>
                <div
                ...
                >
                  <p className="mb-3 text-[14px] font-semibold text-white/70">
                    Recent analyses
                  </p>
          ...
          <motion.div
            ...
            className="flex min-h-0 min-w-0 flex-col max-lg:shrink-0 lg:min-h-full lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0"
          >
            <div className="max-lg:overflow-visible lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:pr-1 app-scrollbar">
              <GlowCard
                className="min-h-0 max-lg:border-0 max-lg:bg-transparent max-lg:shadow-none lg:min-h-[min(100%,32rem)]"
                contentClassName="min-w-0 p-3 sm:p-6 max-lg:px-0 max-lg:py-0"
              >
                {listingAutoBlockedByCv ? (
                  <div className="flex min-h-[220px] flex-col items-center justify-center px-4 py-8 text-center lg:min-h-[480px] lg:py-12">
                  ...
                  <div className="flex min-h-[200px] flex-col items-center justify-center px-4 py-8 text-center lg:min-h-[480px]">
                    <Search className="mb-3 h-12 w-12 text-[#00C9B1] lg:h-16 lg:w-16" />
                            ...
                            <div className="mb-4 flex items-start gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] px-3 py-2.5">
                              ...
                              <div>
                                ...
                                Number.isFinite(displayScoreBeforeTailor) ? (
                                  ...
                                  >
                                    Job fit:{' '}
                                    {Math.round(displayScoreBeforeTailor)}% →{' '}
                                    {Math.round(analysis.matchScore)}
                                    {analysis.matchScore ===
                                    displayScoreBeforeTailor
                                      ? ' (no change)'
                          ...
                          <>
                            ...
                            {!tailorSectionComplete ? (
                              <p className="mb-3 text-xs text-white/50">
                                Select which gaps to address. Critical and
                                high-importance skills are pre-selected.
                              </p>
                            ) : null}
                            <div className="flex flex-col gap-2">
                              {(analysis.missingSkills ?? []).map((skill) => {
                                const checked = selectedSkillNames.includes(
                                  skill.name,
                                );
                                const badgeVariant =
                            ...
                            {analysis && !jobAnalysisIdForTailor ? (
                              <p className="mt-2 text-xs text-amber-200/85">
                                Tailoring needs a saved job id. We try to attach
                                it from Recent analyses automatically — if this
                                stays disabled, click this job in the list on
                                the left, or use{' '}
                            ...
                            {coverLetterEditing ? (
                              <div className="space-y-3">
                                <textarea
                                  ...
                                  onChange={(e) =>
                                    setCoverLetterDraft(e.target.value)
                                  }
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <Button
                                    ...
                                    onClick={() => {
                                      const next =
                                        coverLetterDraft.trim() || null;
                                      setGenerated(next);
                                      setCoverLetterEditing(false);
                                      const jobId = analysis?.id?.trim();
                                      if (jobId && next) {
                                        writeCoverLetterToStorage(jobId, next);
                                        void api.jobs.saveGeneratedCoverLetter(
                                          jobId,
                                          next,
                                        );
                                      }
      ...
      <div
        className="fixed inset-x-0 z-[70] border-t border-white/[0.08] bg-[#080b0a]/95 px-4 py-3 shadow-[0_-8px_24px_rgba(0,0,0,0.35)] backdrop-blur-md lg:hidden"
...
function AnalyzerResultsLoadingShell({
  variant,
}: {
