...
function CVPageContent() {
  ...
  const linkedJobQ = useQuery({
    queryKey: ['job', jobAnalysisIdParam],
  ...
  const targetId = useMemo(() => {
    ...
    const def = profileOptions.find((p) => p.isDefault) ?? profileOptions[0];
  ...
  const createBlank = async (tpl: CvTemplateId) => {
    try {
      ...
      if (id && id !== 'cv-profile') {
        ...
        toast.success('CV created — choose it below to open the editor');
      }
    } catch (e) {
      toast.error(getApiErrorMessage(e));
    }
    ...
    >
      ...
      {targetId ? (
        <AISectionAssistantPanel
          ...
          showFab={!aiChatOpen && !assistantOpen}
      ...
      <Modal
        ...
        description="Upload your CV again to re-extract experience, education, and skills."
        className="max-w-lg"
      >
        <CVUploadZone
          cvProfileId={targetId}
          onSuccess={async ({ profile: p }) => {
