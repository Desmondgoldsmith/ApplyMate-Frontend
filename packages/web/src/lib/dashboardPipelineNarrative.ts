 ...
 */
export function pipelineEyebrowHeadlineRedundant(eyebrow: string, headline: string): boolean {
  ...
  if (e.includes(stem) && h.includes(stem) && Math.abs(e.length - h.length) <= 28) return true;
  return false;
}

/** Merge pipeline + landscape bodies without repeating identical or nested blocks. */
export function mergePipelineLandscapeBodies(a: string | null, b: string | null): string | null {
  const pa = a?.trim() ?? '';
  const pb = b?.trim() ?? '';
  ...
  if (na === nb) return pa;
  if (na.includes(nb) || nb.includes(na)) return pa.length >= pb.length ? pa : pb;
  return `${pa}\n\n${pb}`;
}
...
export function pipelineHeadlineSubsumedByBody(headline: string | null, body: string | null): boolean {
  const h = normLoose(headline ?? '');
  ...
  if (prefixLen >= 16 && h.slice(0, prefixLen) === b.slice(0, prefixLen)) return true;
...
export function dedupeNearDuplicateSentences(text: string): string {
  ...
  const mergedParagraphs = paragraphs.map((para) => {
    const chunks = para.split(/(?<=[.!?])\s+/).map((c) => c.trim()).filter(Boolean);
    const kept: string[] = [];
    for (const c of chunks) {
      const low = normLoose(c);
      const prefixLen = Math.min(32, low.length);
  ...
  for (const p of mergedParagraphs) {
    ...
    const subsumed = out.some((prev) => {
      ...
      return n === pn || (n.length >= 40 && pn.includes(n.slice(0, 40))) || (pn.length >= 40 && n.includes(pn.slice(0, 40)));
    });
